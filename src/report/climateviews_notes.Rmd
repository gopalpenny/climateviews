...
us_counties <- us_counties %>% left_join(ycom_county %>% rename(fips=GEOID),by="fips")
  

# p <- plotly::ggplotly(
p <- ggplot() + 
  geom_sf(data=us_counties,aes(fill=human),color=NA) +
  viridis::scale_fill_viridis("Percent of\npopulation") +
  ggtitle("Is climate change climate change caused by humans?")
  # )
print(p)
```
