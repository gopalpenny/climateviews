...
p_flood <- ggplot() + 
  geom_sf(data=us_counties_data %>% 
            mutate(amount_2000s=log10(amount_2000s),
                   amount_2010s=log10(amount_2010s)) %>% 
  ...
  viridis::scale_fill_viridis("Total paid\n(log $)",option="cividis",limits=c(2,10),breaks=seq(3,9,by=3),labels=c("$1k","$1M","$1B")) +
  facet_wrap(~variable,nrow=1) +
  labs(caption="Data: National Flood Insurance Program (fema.gov/national-flood-insurance-program)") +
  ggtitle(paste0("Flood claims paid by the National Flood Insurance Program (",
